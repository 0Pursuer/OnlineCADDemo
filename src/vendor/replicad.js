...
class Face extends Shape {
  ...
  triangulation(index0 = 0) {
    ...
    const aLocation = r(new this.oc.TopLoc_Location_1());
    const triangulation = r(
      this.oc.BRep_Tool.Triangulation(this.wrapped, aLocation)
    );
    if (triangulation.IsNull())
      return null;
    ...
    // Compute normals: try Poly_Triangulation.Normal() first (available in newer OCCT),
    // fall back to computing from face geometry if not available.
    triangulatedFace.verticesNormals = new Array(nbNodes * 3);
    if (tri.HasNormals()) {
      // Normals are stored directly in the triangulation
      for (let i = 1; i <= nbNodes; i++) {
        const n = r(tri.Normal(i));
        const nt = r(n.Transformed(transformation));
        triangulatedFace.verticesNormals[(i - 1) * 3 + 0] = nt.X();
        triangulatedFace.verticesNormals[(i - 1) * 3 + 1] = nt.Y();
        triangulatedFace.verticesNormals[(i - 1) * 3 + 2] = nt.Z();
      }
    } else {
      // Fallback: use BRepGProp_Face to compute the normal at face center
      // and apply it uniformly to all vertices
      const props = r(new this.oc.BRepGProp_Face_2(this.wrapped, false));
      const { uMin, uMax, vMin, vMax } = this.UVBounds;
      const midU = 0.5 * (uMin + uMax);
      const midV = 0.5 * (vMin + vMax);
      const pTmp = r(new this.oc.gp_Pnt_1());
      const nTmp = r(new this.oc.gp_Vec_1());
      props.Normal(midU, midV, pTmp, nTmp);
      const nx = nTmp.X(), ny = nTmp.Y(), nz = nTmp.Z();
      for (let i = 0; i < nbNodes; i++) {
        triangulatedFace.verticesNormals[i * 3 + 0] = nx;
        triangulatedFace.verticesNormals[i * 3 + 1] = ny;
        triangulatedFace.verticesNormals[i * 3 + 2] = nz;
      }
    }
...
class _3DShape extends Shape {
  ...
  fuse(other, {
    ...
    const r = GCWithScope();
    const argList = r(new this.oc.TopTools_ListOfShape_1());
    argList.Append_1(this.wrapped);
    const toolList = r(new this.oc.TopTools_ListOfShape_1());
    toolList.Append_1(other.wrapped);
    const newBody = r(new this.oc.BRepAlgoAPI_Fuse_1());
    newBody.SetArguments(argList);
    newBody.SetTools(toolList);
  ...
  cut(tool, {
    ...
    argList.Append_1(this.wrapped);
    const toolList = r(new this.oc.TopTools_ListOfShape_1());
    toolList.Append_1(tool.wrapped);
    const cutter = r(new this.oc.BRepAlgoAPI_Cut_1());
  ...
  intersect(tool) {
    ...
    const toolList = r(new this.oc.TopTools_ListOfShape_1());
    toolList.Append_1(tool.wrapped);
    const intersector = r(new this.oc.BRepAlgoAPI_Common_1());
    intersector.SetArguments(argList);
    intersector.SetTools(toolList);
    intersector.Build();
...
const assembleWire = (listOfEdges) => {
  ...
  console.log("[PATCH-TEST] Calling wireBuilder.Build() at line 3745");
  wireBuilder.Build();
